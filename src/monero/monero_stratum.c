                                               ...
                                               struct stratum_event_handler *event_handler)
{
  ...
  if(err_msg != NULL) {
    ...
    struct stratum_event_login_failed event = {
      .stratum_event = {STRATUM_EVENT_LOGIN_FAILED},
      .error = "Server returned status is not \"OK\", but error message was not provided"
    };
    event_handler->cb(&event.stratum_event, event_handler->data);
  } else if(miner_id_json == NULL) {
    struct stratum_event_invalid_reply event = {
      .stratum_event = {STRATUM_EVENT_INVALID_REPLY},
      .error = "Failed to parse server reply: expected miner \"id\" field is missing"
    };
    event_handler->cb(&event.stratum_event,event_handler->data);
  } else {
...
void monero_stratum_handle_json_response(const cJSON *json,
                                         struct stratum_event_handler *event_handler)
{
  ...
  if (error_json != NULL && !cJSON_IsNull(error_json)) {
    ...
    err_msg = (err_json != NULL && cJSON_IsString(err_json)) ?
      err_json->valuestring : "Server returned unparsable error";
  ...
  case MONERO_STRATUM_MESSAGE_TYPE_LOGIN:
    monero_stratum_handle_json_login_response(result_json, err_msg, event_handler);
    break;
  default:
...
void monero_stratum_handle_json_request(const char *method,
                                        const cJSON *params_json,
                                        struct stratum_event_handler *event_handler)
{
...
void monero_stratum_handle_json(const cJSON *json, struct stratum_event_handler *event_handler)
{
  cJSON* method_json = cJSON_GetObjectItem(json, "method");
  if(method_json != NULL && cJSON_IsString(method_json)) {
    const char *method = method_json->valuestring;
    monero_stratum_handle_json_request(method, cJSON_GetObjectItem(json, "params"), event_handler);
  } else {
                          ...
                          struct stratum_event_handler *event_handler)
{
  ...
  const char *login_cmd = "{\"id\":%d,\"jsonrpc\":\"2.0\",\"method\":\"login\",\"params\":{"
    "\"login\":\"%s\","
    "\"pass\":\"%s\","
    "\"agent\":\"%s\""
    "}}\n";
  ...
  int len = snprintf(buf.base, buf.len, login_cmd,
                     MONERO_STRATUM_MESSAGE_TYPE_LOGIN,
...
void monero_stratum_new_payload(stratum_handle stratum, const uv_buf_t *buf)
{
  ...
  if(json == NULL) {
    ...
    monero_stratum->stratum_event_handler->cb(&event.stratum_event,
                                              monero_stratum->stratum_event_handler->data);
    return;
  }
